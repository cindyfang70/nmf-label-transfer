...
test_that("single sample label transfer works", {
  ...
  #                          technicalVarName="sample_id", save_nmf=FALSE, k=10, tol=1e-5)
  #
  # #print(preds)
  expect_equal(dim(vis_anno_target), dim(target_with_preds))
  expect_true(length(res$targets$nmf_preds)==ncol(vis_anno_target))
})
